...
exports.getAttendance = function getAttendance(rollNo, section, subject) {
  return new Promise((resolve, reject) => {
    let sql = `select count(a${rollNo}) from section_${section}_attendance where lecture like '${subject}%' and a${rollNo}='Present'`;
    console.log(sql);
...
exports.getTotalLectures = function getTotalLectures(
  subject,
  section,
  department
) {
  return new Promise((resolve, reject) => {
    ...
    database.query(
      `select total_lectures from department_${department} where Subject='${subject}' and Section='${section}'`,
      (err, result) => {
        if (err) reject("sql error");
