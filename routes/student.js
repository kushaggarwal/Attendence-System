...
router.post("/checkAttendance", (req, res) => {
  ...
  util.getConnection().query(`select subject from department_${department} where section='${section}'`, async (err, result) => {
    ...
    if(!result) {
      res.render("checkAttendance", {error_msg: "Invalid department"});
      return;
    }
    else if(result.length === 0) {
      res.render("checkAttendance", {error_msg: "Invalid section "});
      return;
    }
    console.log(result);
    ...
    util.getConnection().query(`select studentName from section_${section} where studentRollNo='${rollNo}'`, async (err, result) => {
      if(result.length == 0) {
        ...
        res.render("checkAttendance", {error_msg: "Stundent Roll Number not listed in given department and section"})
        return;
      }
      else {
